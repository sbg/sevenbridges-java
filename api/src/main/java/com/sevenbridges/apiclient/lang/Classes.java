...
public class Classes {
  ...
  private Classes() { /* No instance methods */ }
  ...
  /**
   * Attempts to load the specified class name from the current thread's {@link
   * Thread#getContextClassLoader() context class loader}, then the current ClassLoader ({@code
   * Classes.class.getClassLoader()}), then the system/application ClassLoader ({@code
   * ClassLoader.getSystemClassLoader()}, in that order.  If any of them cannot locate the specified
   * class, an {@code UnknownClassException} is thrown (our RuntimeException equivalent of the JRE's
   * {@code ClassNotFoundException}.
   *
   * @param <T>  type of class to get for fully qualified name
  ...
  public static <T> Class<T> forName(String fqcn) throws UnknownClassException {
    ...
    if (clazz == null) {
      ...
      if (fqcn != null && fqcn.startsWith("com.sevenbridges.apiclient.impl")) {
        msg += "  Have you remembered to include the sevenbridges-java-impl.jar in your runtime classpath?";
      }
   ...
   * Thread#getContextClassLoader() context class loader}, then the current ClassLoader ({@code
   * Classes.class.getClassLoader()}), then the system/application ClassLoader ({@code
   * ClassLoader.getSystemClassLoader()}, in that order, using {@link
   * ClassLoader#getResourceAsStream(String) getResourceAsStream(name)}.
   *
   * @param name the name of the resource to acquire from the classloader(s).
   * @return the InputStream of the resource found, or {@code null} if the resource cannot be found
   * from any of the three mentioned ClassLoaders.
   */
