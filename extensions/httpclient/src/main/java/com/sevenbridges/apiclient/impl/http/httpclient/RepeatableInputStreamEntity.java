 ...
 * {@link InputStreamEntity}, with the one notable difference, that if the underlying InputStream
 * supports being reset, this RequestEntity will report that it is repeatable and will reset the
 * stream on all subsequent attempts to write out the request.
 */
class RepeatableInputStreamEntity extends BasicHttpEntity {
  ...
  /**
   * Creates a new RepeatableInputStreamEntity using the information from the specified request. If
   * the input stream containing the request's contents is repeatable, then this RequestEntity will
   * report as being repeatable.
   *
   * @param request The details of the request being written out (content type, content length, and
   *                content).
   */
   ...
   * InputStreamRequestEntity is repeatable (i.e. its entire contents will be buffered in memory and
   * can be repeated).
   */
  @Override
