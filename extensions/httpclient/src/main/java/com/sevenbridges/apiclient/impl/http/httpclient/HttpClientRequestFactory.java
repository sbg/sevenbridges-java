...
class HttpClientRequestFactory {
  ...
  private void applyHeaders(HttpRequestBase httpRequest, Request request) {
    /*
     * Apache HttpClient omits the port number in the Host header (even if we explicitly specify it)
     * if it's the default port for the protocol in use. To ensure that we use the same Host header
     * in the request and in the calculated string to sign (even if Apache HttpClient changed and
     * started honoring our explicit host with endpoint), we follow this same behavior here and in
     * the RequestAuthenticator.
     */
    ...
    for (Map.Entry<String, List<String>> entry : request.getHeaders().entrySet()) {
      ...
      /*
       * HttpClient4 fills in the Content-Length header and complains if it's already present, so we
       * skip it here. We also skip the Host header to avoid sending it twice, which will interfere
