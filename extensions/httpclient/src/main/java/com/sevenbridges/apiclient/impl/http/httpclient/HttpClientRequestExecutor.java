 ...
 */
package com.sevenbridges.apiclient.impl.http.httpclient;
...
import com.sevenbridges.apiclient.impl.http.RestException;
import com.sevenbridges.apiclient.impl.http.authc.DefaultRequestAuthenticatorFactory;
import com.sevenbridges.apiclient.impl.http.authc.RequestAuthenticator;
import com.sevenbridges.apiclient.impl.http.authc.RequestAuthenticatorFactory;
import com.sevenbridges.apiclient.impl.http.support.BackoffStrategy;
import com.sevenbridges.apiclient.impl.http.support.DefaultRequest;
import com.sevenbridges.apiclient.impl.http.support.DefaultResponse;
...
/**
 * {@code RequestExecutor} implementation that uses the <a href="http://hc.apache.org/httpcomponents-client-ga">Apache
 * HttpClient</a> implementation to execute http requests.
 */
public class HttpClientRequestExecutor implements RequestExecutor {
  ...
  private static final String MAX_CONNECTIONS_PER_ROUTE_PROPERTY_KEY = "com.sevenbridges.apiclient.impl.http.httpclient.HttpClientRequestExecutor.connPoolControl.maxPerRoute";
  ...
  private static final String MAX_CONNECTIONS_TOTAL_PROPERTY_KEY = "com.sevenbridges.apiclient.impl.http.httpclient.HttpClientRequestExecutor.connPoolControl.maxTotal";
  ...
  /**
   * Creates a new {@code HttpClientRequestExecutor} using the specified {@code ApiKey} and optional
   * {@code Proxy} configuration.
   *
   * @param apiKey               the SevenBridges account API Key that will be used to authenticate
   *                             the client with SevenBridges's API sever
   ...
   *                             the SevenBridges API server. If null, then Sauthc1 will be used.
   */
  ...
  /**
   * Returns {@code true} if the exception resulted from a throttling error, {@code false}
   * otherwise.
   *
   * @param re The exception to test.
   * @return {@code true} if the exception resulted from a throttling error, {@code false}
   * otherwise.
   */
  ...
  protected Response toSdkResponse(HttpResponse httpResponse) throws IOException {
    ...
    Response response = new DefaultResponse(httpStatus, headers, body, contentLength);
